...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^mm_") ; then
       MM_BUILD=$(echo -n $1 | sed -e 's/^mm_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $MM_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       MM_BUILD=
    fi
    export MM_BUILD
...
{
    target=$1
    MM_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/mm/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var MM_VERSION)
        ZIPFILE=mm-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.mm.device=$MM_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $MM_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/mm/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
